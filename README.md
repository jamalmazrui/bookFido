...
bookFido — the dog that fetches your books — catalogs an Audible library for screen reader users.  It is part of the Homer Tools series, a sibling of [urlFido](https://github.com/jamalmazrui/urlFido).  It signs into Audible through your own Microsoft Edge profile, walks every page of the library, downloads each companion PDF under a friendly name, converts each PDF to accessible HTML, gathers details about every title from Audible's catalog service, Open Library, and Wikipedia, including author biographies, and writes the catalog to your Downloads folder as Audible_Library.htm and Audible_Library.md (Pandoc-flavored Markdown), with a sortable sheet for every library in one bookFido.xlsx workbook.  Kindle books on the same Amazon account are cataloged alongside, as Kindle_Library.htm, Kindle_Library.md, and Kindle_Library.xlsx, with every field the Kindle library service offers; the Goodreads My Books shelves and the Bookshare My History list are cataloged as Goodreads_Library and Bookshare_Library documents with every field their pages offer; a title present in more than one library shares its gathered details, a shared author's biography is fetched only once, and everything gathered is remembered in bookFido.db beside the program, a SQLite database in the standard DbDo schema, so later runs never search for what is already known and the database opens directly in DbDo.
